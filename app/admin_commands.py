...
async def cmd_fiche(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with db_session() as s:
        ...
        await update.message.reply_text(
            ...
            f"• *Recherche* : {p.sought_age_min or '?'}-{p.sought_age_max or '?'} ans, "
            f"{p.sought_location or '—'} ; pratique : {p.sought_religious or '—'} ; "
            f"enfants : {oui_non.get(p.sought_wants_children, '—')} ; "
            f"qualités : {', '.join(p.sought_qualities or []) or '—'}\n"
